...
def income_expense_entry(): # Function to allow user to add, delete or update an entry into the tables 'income' and 'expense'.
    ...
    if user_response == 'A':                                                                                    # In the first block, the user is requested to 
        ...
        if user_choice == 'I':                                                                                  # is added to the table with the current date as the
            income_entry = []                                                                                   # date of the entry.
        ...
        if user_choice == 'I':                                                                                  # the date of the entry that needs to be deleted
            cursor.execute('''SELECT * FROM income''')                                                          # and the program removes it from the table.
            print('Listing incomes by date...')
            print(tabulate(cursor.fetchall(), headers=income_column_names))
        ...
        if user_choice == 'I':                                                                                  # to be updated. After that, they are asked to 
            cursor.execute('''SELECT * FROM income''')                                                          # introduce new data for some of the categories
            print('Listing incomes by date...')                                                                 # and the program updates the entry with the new data.
            print(tabulate(cursor.fetchall(), headers=income_column_names))
            new_income_entry = []
